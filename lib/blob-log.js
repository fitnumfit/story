...
/** User-written answers — one folder per unique user: her-responses/{userId}/ */
const USER_RESPONSE_EVENTS = new Set(["accept_apology", "smile", "note"]);

/** Page views & navigation — book-activity/{userId}/ */
...
function blobFolder(event, userId) {
  const uid = safeUserId(userId);
  if (USER_RESPONSE_EVENTS.has(event)) return `her-responses/${uid}`;
  if (ANALYTICS_EVENTS.has(event)) return `book-activity/${uid}`;
...
export async function saveUserSnapshot({ sessionId, userId, times, event, data, meta }) {
  ...
  return putToBlob(`her-responses/${uid}/summary.json`, JSON.stringify(body, null, 2), {
...
export async function saveEventToBlob({ event, sessionId, userId, times, data, meta }) {
  ...
  const folder = blobFolder(event, userId);
